...
class LatexSplitter(BaseTextSplitter):
    """
    ...
    """
    def __init__(self, chunk_size: int = 100, chunk_overlap: int = 0, separators: Optional[List[str]] = None,
        ...
        self.markdown_splitter = LatexTextSplitter(chunk_size=chunk_size, chunk_overlap=chunk_overlap,separators=separators, keep_separator= keep_separator)

    def split_document(self, document: Document):
        doc_copy = copy.deepcopy(document)
        split_documents = self.markdown_splitter.split_documents(document.page_content)
